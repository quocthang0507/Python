...
def background_2(player, playerpos):
    for x in range(int(width/grass.get_width())+1):
        for y in range(int(height/grass.get_height())+1):
            screen.blit(grass, (x*100, y*100))
    screen.blit(player, playerpos)
...
def mouseMove():
    ...
    _player = pygame.transform.rotate(player, 360-angle*57.29)
    _playerpos = [playerpos[0]-_player.get_rect().width/2,
                  playerpos[1]-_player.get_rect().height/2]
    background_2(_player, _playerpos)
